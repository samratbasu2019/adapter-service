...
public class AdapterController {
	...
	@PostMapping("/adapter/jira/pull/tasks")
	public ResponseEntity<?> pullJiraTasks(@RequestParam String email) {
		...
		try {
			...
			return new ResponseEntity<>(ResponseHelper.populateRresponse(e.getStackTrace().toString(), "failure"),
					HttpStatus.BAD_REQUEST);
		}
		return new ResponseEntity<>(ResponseHelper.populateRresponse("Jira tasks pulled sucessfully", "Success"),
				HttpStatus.OK);
	}
	...
	public ResponseEntity<?> uploadAppreciation(@RequestParam(value = "files", required = false) MultipartFile[] files,
		...
		if (iCountStore.getAppreciation() != null) {
			...
			index = 0;
			return new ResponseEntity<>(ResponseHelper.populateRresponse("Bad request", "failed"),
					HttpStatus.BAD_REQUEST);
		}
		index = 0;
